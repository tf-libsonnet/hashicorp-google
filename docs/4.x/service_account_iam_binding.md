...
**Args**:
  ...
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.service_account_iam_binding.condition.new](#fn-service_account_iam_bindingconditionnew) constructor.
...
Unlike [google.service_account_iam_binding.new](#fn-service_account_iam_bindingnew), this function will not inject the `resource`
