...
**Args**:
  ...
  - `member` (`string`): 
  - `role` (`string`): 
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.healthcare_dicom_store_iam_member.condition.new](#fn-healthcare_dicom_store_iam_memberconditionnew) constructor.
...
Unlike [google.healthcare_dicom_store_iam_member.new](#fn-healthcare_dicom_store_iam_membernew), this function will not inject the `resource`
