...
**Args**:
  ...
  - `role` (`string`): 
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.iap_web_type_app_engine_iam_binding.condition.new](#fn-conditionnew) constructor.
...
Unlike [google.iap_web_type_app_engine_iam_binding.new](#fn-new), this function will not inject the `resource`
