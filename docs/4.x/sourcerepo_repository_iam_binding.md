...
**Args**:
  ...
  - `repository` (`string`): 
  - `role` (`string`): 
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.sourcerepo_repository_iam_binding.condition.new](#fn-sourcerepo_repository_iam_bindingconditionnew) constructor.
...
Unlike [google.sourcerepo_repository_iam_binding.new](#fn-sourcerepo_repository_iam_bindingnew), this function will not inject the `resource`
