...
Format: accessPolicies/{policy_id}
  - `access_levels` (`list[obj]`): The desired Access Levels that should replace all existing Access Levels in the Access Policy. When `null`, the `access_levels` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.access_context_manager_access_levels.access_levels.new](#fn-access_context_manager_access_levelsaccess_levelsnew) constructor.
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.access_context_manager_access_levels.timeouts.new](#fn-access_context_manager_access_levelstimeoutsnew) constructor.
...
Unlike [google.access_context_manager_access_levels.new](#fn-access_context_manager_access_levelsnew), this function will not inject the `resource`
