...
**Args**:
  ...
  - `timeouts` (`obj`):  When `null`, the `timeouts` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.firestore_document.timeouts.new](#fn-firestore_documenttimeoutsnew) constructor.
...
Unlike [google.firestore_document.new](#fn-firestore_documentnew), this function will not inject the `resource`
