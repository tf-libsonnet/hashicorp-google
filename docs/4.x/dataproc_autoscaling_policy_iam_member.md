...
**Args**:
  ...
  - `role` (`string`): 
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.dataproc_autoscaling_policy_iam_member.condition.new](#fn-dataproc_autoscaling_policy_iam_memberconditionnew) constructor.
...
Unlike [google.dataproc_autoscaling_policy_iam_member.new](#fn-dataproc_autoscaling_policy_iam_membernew), this function will not inject the `resource`
