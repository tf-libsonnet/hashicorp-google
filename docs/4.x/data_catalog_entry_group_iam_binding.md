...
**Args**:
  ...
  - `role` (`string`): 
  - `condition` (`list[obj]`):  When `null`, the `condition` sub block will be omitted from the resulting object. When setting the sub block, it is recommended to construct the object using the [google.data_catalog_entry_group_iam_binding.condition.new](#fn-data_catalog_entry_group_iam_bindingconditionnew) constructor.
...
Unlike [google.data_catalog_entry_group_iam_binding.new](#fn-data_catalog_entry_group_iam_bindingnew), this function will not inject the `resource`
